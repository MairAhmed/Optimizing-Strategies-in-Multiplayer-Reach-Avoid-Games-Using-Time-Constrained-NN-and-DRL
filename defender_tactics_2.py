...
class ReachAvoidEnv(gym.Env):
    def __init__(self, region_size_x=2.0, region_size_y=1.5, target_shape="circle", target_size=0.05, max_time_steps=1000):
        ...
        self.target_position = np.array([self.play_region_size_x - 0.1, self.play_region_size_y - 0.1])  # Objective at opposite corner
        ...
        self.attacker_position = np.array([0.1, 0.1])  # Starting position of the attacker
        ...
        self.defender_positions = [
            np.array([self.play_region_size_x / 2, self.play_region_size_y / 2]),  # Defenders in the center
            np.array([self.play_region_size_x / 2, self.play_region_size_y / 2])
        ]
        self.defender_speeds = [0.01, 0.01]  # Very low speed for defenders
        self.defender_detection_range = 0.5  # Detection range for defenders to start chasing
    ...
    def create_static_obstacles(self):
        ...
        static_obstacles = [
            p.createMultiBody(baseMass=0, baseCollisionShapeIndex=p.createCollisionShape(p.GEOM_BOX, halfExtents=[0.05, 0.05, 0.05]),
                              basePosition=[0.5, 0.6, 0]),  # Box near the target
            p.createMultiBody(baseMass=0, baseCollisionShapeIndex=p.createCollisionShape(p.GEOM_BOX, halfExtents=[0.05, 0.05, 0.05]),
                              basePosition=[0.6, 0.5, 0])  # Another box near the target
        ]
    ...
    def reset(self):
        ...
        self.attacker_position = np.array([0.1, 0.1])  # Attacker starts at one corner
    ...
    def check_collisions(self, new_position_x, new_position_y):
        """Check for collisions between the attacker and obstacles (static and dynamic)."""
        for obstacle in self.static_obstacles + self.dynamic_obstacles:
            obstacle_position = p.getBasePositionAndOrientation(obstacle)[0]
            distance_to_obstacle = np.linalg.norm([new_position_x - obstacle_position[0], new_position_y - obstacle_position[1]])
            if distance_to_obstacle <= 0.1:  # Assuming a small radius for collision
                return True
        return False

    def check_defender_collisions(self):
        """Check for collisions between the attacker and the defenders."""
        for defender_position in self.defender_positions:
            distance_to_defender = np.linalg.norm(self.attacker_position - defender_position)
            if distance_to_defender <= 0.1:  # Assuming a small radius for collision
                return True
        return False

    def update_defender_positions(self):
        """Update the defender positions to either move randomly or chase the attacker."""
        for i in range(len(self.defender_positions)):
            defender_pos = self.defender_positions[i]

            # Random movement
            random_angle = np.random.uniform(0, 2 * np.pi)
            defender_pos += 0.005 * np.array([np.cos(random_angle), np.sin(random_angle)])  # Slower speed

            # Check if the attacker is within the detection range
            distance_to_attacker = np.linalg.norm(defender_pos - self.attacker_position)
            if distance_to_attacker < self.defender_detection_range:
                # Move defender towards the attacker
                direction = self.attacker_position - defender_pos
                direction /= np.linalg.norm(direction)  # Normalize the direction vector
                defender_pos += direction * self.defender_speeds[i]  # Move towards the attacker

            # Keep defenders within the play region boundaries
            defender_pos[0] = np.clip(defender_pos[0], 0, self.play_region_size_x)
            defender_pos[1] = np.clip(defender_pos[1], 0, self.play_region_size_y)

            self.defender_positions[i] = defender_pos
    ...
    def step(self, action):
        ...
        # Collision check with obstacles: If the attacker hits an obstacle, adjust the heading
        if self.check_collisions(new_position_x, new_position_y):
            self.attacker_heading += np.pi  # Reflect the heading if there's a collision
        ...
        # Move defenders
        self.update_defender_positions()

        # Check if the attacker collides with a defender
        if self.check_defender_collisions():
            done = True  # End the game on defender collision
        else:
...
env = ReachAvoidEnv(region_size_x=2.0, region_size_y=1.5, target_shape="circle", target_size=0.05, max_time_steps=500)
env.visualize()
env.close()
